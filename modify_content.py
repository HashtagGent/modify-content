...
def process_file():
    ...
    # Always save with the correct extension
    input_path = os.path.join(UPLOAD_FOLDER, f"{timestamp}.{file_type}")

    file.save(input_path)

    if file_type == "mp4":
        output_path = os.path.join(OUTPUT_FOLDER, f"{timestamp}_modified.mp4")
        output_path = modify_video(input_path, output_path)
        mime_type = "video/mp4"
        download_name = f"{timestamp}_modified.mp4"

    elif file_type in ["jpg", "png"]:
        output_path = os.path.join(OUTPUT_FOLDER, f"{timestamp}_modified.jpg")  # Ensure .jpg extension
        output_path = modify_photo(input_path, output_path)
        mime_type = "image/jpeg"
        download_name = f"{timestamp}_modified.jpg"  # Ensure correct filename
